...
var archiveApp = require('./archive.js');
var os = require('os');
...
Actions.prototype.build = function() {
  var self = this;
  var argIndex = 3;
  var skipArchiving = false;
  if (process.argv[argIndex].match('--skip-archive')) {
    argIndex++;
    skipArchiving = true;
  }
  var buildLocation = process.argv.slice(argIndex).join(" ");
  if (buildLocation === "")
  {
    console.error("path to build location must be defined when using the 'build' & 'deploy-build' commands".red.bold);
    helpers.printHelp();
    process.exit(1);
  }
  ...
  buildApp(appPath, meteorBinary, buildLocation, skipArchiving, function(err) {
    if(err) {
      process.exit(1);
    } else {
      console.log('Build Completed: ' + skipArchiving ? bundlePath : '');
    }
  });
};

Actions.prototype.archive = function() {
  var self = this;
  var buildLocation = process.argv.slice(3).join(" ");
  if (buildLocation === "") {
    console.error("path to build location must be defined when using the 'archive' command".red.bold);
    helpers.printHelp();
    process.exit(1);
  }
  ...
  console.log('Archive Started: ' + this.config.app);
  console.log('Build location: ' + buildLocation);
  archiveApp(buildLocation, function(err) {
    if(err) {
      process.exit(1);
    } else {
      console.log('Archive Completed: ' + bundlePath);
    }
  });
};

Actions.prototype.release = function() {
  var self = this;
  var buildLocation = process.argv.slice(3).join(" ");
  if (buildLocation === "")
  {
...
Actions.init = function() {
  ...
  if (!fs.existsSync(destSettingsJson))
