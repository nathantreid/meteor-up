...
Actions.prototype.build = function() {
  var self = this;
  if (typeof this.config.buildLocation === "undefined")
  {
    console.error("buildLocation variable must be defined when using the 'build' & 'deploy-build' commands".red.bold);
    process.exit(1);
  }
  self._showKadiraLink();

  var buildLocation = this.config.buildLocation;
  var bundlePath = path.resolve(buildLocation, 'bundle.tar.gz');
  ...
  buildApp(appPath, meteorBinary, buildLocation, function(err) {
    if(err) {
      ...
      console.log('Build Completed: ' + bundlePath);
    }
  });
};

Actions.prototype['deploy-build'] = function() {
  var self = this;
  if (typeof this.config.buildLocation === "undefined")
  {
    console.error("buildLocation variable must be defined when using the 'build' & 'deploy-build' commands".red.bold);
